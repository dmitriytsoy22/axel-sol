...
7. **The app is not wired to every backend flow.** It has no Sumsub KYC flow, and the console does not call the operator's deposit flow (`POST /v2/deposits/draft`). It does not show the telemetry widget's `dataOrigin`. The app reads published car data from one base URL, so a deployment verifies either the seed's cars (`/demo-data`) or the backend's (`<backend>/published`), not both. The backend does not publish the purchase documents of its cars.
